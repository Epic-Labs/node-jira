...
(function() {
    ...
    this.getLastSprintForRapidView = function(rapidViewId, callback) {
        ...
        this.doRequest(options, function(error, response) {
          ...
          if (response && response.body && response.body.sprints) {
            var sprints = response.body.sprints || [];
            callback(null, sprints.pop());
            return;
          }

          //just return with empty sprint
          return callback(null);
        });
    };

    this.getSprintBurndown = function(rapidViewId, sprintId, callback) {
      var options = {
        rejectUnauthorized: this.strictSSL,
        url: this.makeUri('/rapid/charts/scopechangeburndownchart.json?rapidViewId=' + rapidViewId + '&sprintId=' + sprintId, 'rest/greenhopper/'),
        method: 'GET',
        json: true
      };
    ...
    this.findIssueChangelog = function(issueNumber, callback) {

        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/issue/' + issueNumber + '?expand=changelog'),
            method: 'GET'
        };
